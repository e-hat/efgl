...
namespace efgl {
	Texture2D::Texture2D(unsigned int rendererID, TextureType type)
	...
	Texture2D TextureManager::LoadTexture(const std::string& path, const std::string& directory, TextureType type) {
		...
		Texture2D result(rendererID, type);

...
// params for indirect dispatch
typedef struct DispatchIndirectParams 
{
	unsigned int x;
	unsigned int y;
	unsigned int z;
...
namespace efgl 
{
	Renderer::Renderer(Ref<Scene> scene)
		: m_Scene(scene), m_Camera(m_Scene->Camera)
	{
		...
		m_DepthMap = MakeRef<Texture>(
			m_Camera.ScreenWidth, m_Camera.ScreenHeight, GL_DEPTH_COMPONENT
		);
		...
		m_Shader->Bind();
		m_Shader->SetUniform("proj", m_Camera.GetProjectionMatrix());
		m_Shader->Unbind();
	}
	...
	void Renderer::colorPass() {
		...
		TracyGpuZoneC("Color pass", tracy::Color::SkyBlue);
		
		m_nActiveLights->SetData(0, sizeof(unsigned int), &N_ACTIVE_LIGHTS_CLEAR_VAL);
		m_nActiveClusters->SetData(0, sizeof(DispatchIndirectParams), &N_ACTIVE_CLUSTERS_CLEAR_VAL);
	...
	void Renderer::depthPass() 
	{
		...
		TracyGpuZoneC("Depth pass", tracy::Color::Gold);
	...
	void Renderer::makeSSBOs() 
	{
		...
		auto nGlobalActiveClusters = MakeRef<DispatchIndirectParams>(N_ACTIVE_CLUSTERS_CLEAR_VAL);
		m_nActiveClusters = MakeRef<ShaderStorageBuffer>(
			sizeof(DispatchIndirectParams), 1, nGlobalActiveClusters.get(), GL_DYNAMIC_DRAW
		);
		m_nActiveClusters->Bind(N_ACTIVE_CLUSTERS_BINDING);
		m_nActiveClusters->BindToTarget(GL_DISPATCH_INDIRECT_BUFFER);
		...
		std::vector<LightGridEntry> defaultEntries(NUM_CLUSTERS);
		for (int i = 0; i < NUM_CLUSTERS; ++i) 
		{
			defaultEntries[i] = (LightGridEntry{ 0, 0 });
		}
		...
		m_nActiveLights = MakeRef<ShaderStorageBuffer>(
			sizeof(unsigned int), 1, nActiveLights.get(), GL_DYNAMIC_DRAW
		);
